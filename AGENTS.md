...
- `install.sh` (Linux+macOS) + `install.ps1` (Windows): idempotent `PATH`, prompt `GROQ_API_KEY` → `~/.config/voice2text/groq.key` (Unix, raw, `chmod 600`) / `setx` (Win), `WScript.Shell .lnk` (Win, only hotkey creator)
...
Verify:
```sh
read -r GROQ_API_KEY <~/.config/voice2text/groq.key 2>/dev/null; bash -x ./voice2text.sh & sleep 0.5; pgrep -a arecord; pkill -2 -f "arecord -q -t raw -f S16_LE -r 16000"; sleep 0.5; ls -l ${XDG_RUNTIME_DIR:-/tmp}/v2t-*/v2t.ogg; curl -sS $API -H "Bearer $GROQ_API_KEY" -F file=@... | jq
```
...
- Keep installers idempotent: `grep -q PATH || echo`, `D=$(cd $(dirname $0) && pwd)`, `read -r` + `chmod 600`, raw `groq.key` (no `export`, no quoting)
